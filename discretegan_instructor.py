...
from models.DISCRETEGAN_D import DISCRETEGAN_D
from models.DISCRETEGAN_G import DISCRETEGAN_G
from utils import rollout
from utils.data_loader import GenDataIter, DisDataIter


class DISCRETEGANInstructor(BasicInstructor):
    def __init__(self, opt):
        super(DISCRETEGANInstructor, self).__init__(opt)

        # generator, discriminator
        self.gen = DISCRETEGAN_G(cfg.gen_embed_dim, cfg.gen_hidden_dim, cfg.vocab_size, cfg.max_seq_len,
                            cfg.padding_idx, gpu=cfg.CUDA)
        self.dis = DISCRETEGAN_D(cfg.dis_embed_dim, cfg.vocab_size, cfg.padding_idx, gpu=cfg.CUDA)
        self.init_model()
    ...
    def adv_train_generator(self, g_step):
        """
        dailu: embedding-search based gradient estimator
        """
        ...
        for step in range(g_step):
            ...
            if self.gen.gpu:
                inp = inp.cuda()
            for idx in range(self.gen.max_seq_len):
                pred, hidden = self.gen.forward(inp, hidden, need_hidden=True) # b x v
                sample = torch.multinomial(torch.exp(pred), 1).squeeze(-1) # pred is in log format, thus exp(b,)
                samples.append(sample)
                preds.append(pred)
            
            samples = torch.stack(samples, dim=1) #b x max_seq_len
            #TODO: need to add mask of EOS tokens
            e_gradient = self.dis.get_gradient_estimation(samples) #b * max_seq_len * v

            preds = torch.stack(preds, dim=1) # b x max_seq_len x v
            adv_loss = -torch.sum(e_gradient * preds) # need to add instead of minus
            self.optimize(self.gen_adv_opt, adv_loss, model=self.gen)
            total_g_loss += adv_loss.item()
            # get estimated gradient from discriminator
